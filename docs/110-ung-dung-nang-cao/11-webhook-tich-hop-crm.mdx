...
Webhook là tính năng tự động gửi dữ liệu real-time từ Subiz sang bên thứ ba \(CRM, SMS, Google Sheet...\) ngay khi có sự kiện phát sinh như: có tin nhắn mới, có khách hàng điền form, tư vấn viên cập nhật thông tin,...
...
- Khi khách hàng được cập nhật thông tin trên Subiz. Webhook gửi thông tin này sang server của bạn.

![](https://vcdn.subiz-cdn.com/file/fismoifcthxupvsvfmzk_acpxkgumifuoofoosble/9b03afb5a9a5aa48c4e380733f0e2db0.png)

Subiz hỗ trợ nhiều loại sự kiện liên quan đến hội thoại, người dùng \(khách hàng\), theo dõi website và đơn hàng:![](https://vcdn.subiz-cdn.com/file/fisphxuxswohswlswqxc_acpxkgumifuoofoosble/c2db8ba42ea45c37f81596a37f2f7e8e.png)
...
Chữ ký được tạo ra bằng <b>thuật toán HMAC với hàm hash SHA256</b> \([hash-based message authentication code](https://en.wikipedia.org/wiki/HMAC) - là một kỹ thuật cơ bản và hiệu quả, hiện đang được sử dụng rộng rãi trên hầu hết các nền tảng webhook như của facebook, google, stripe hay github\). HMAC được triển khai sẵn trên hầu hết các nền tảng lập trình. Về cơ bản, HMAC là một hàm nhận vào 3 tham số: thuật toán hash, mật khẩu và nội dung gói tin, trả ra chữ ký cho gói tin truyền vào.
...
1. Lấy mật khẩu và nội dung body của gói tin cần gửi đi, chạy qua thuật toán HMAC-sha256. Kết quả nhận được chính là chữ ký.
2. Đưa chữ ký vào trong HTTP header X-Hub-Signature-256. Ví dụ X-Hub-Signature-256: sha256=766c10dbe4387
3. Gửi HTTP request tới endpoint

Đầu nhận \(endpoint của bạn\)

1. Khi nhận được request mới, đọc HTTP header X-Hub-Signature-256, bóc tách để lấy chữ ký cho gói tin nhận được.
2. Lấy mật khẩu và nội dung body của gói tin nhận được, chạy qua thuật toán HMAC-sha256 để lấy chữ ký chuẩn.
3. So sánh chữ ký vừa sinh ra và <b>những</b> chữ ký nhận được. Nếu giống hệt nhau có thì gói tin đáng tin cậy. Nếu khác nhau chứng tỏ gói tin nhận được không phải từ subiz, có thể loại bỏ.
...
:::info
Gói tin sẽ bao gồm rất nhiều dữ liệu liên quan tới sự kiện bạn quan tâm. Hãy thử tạo 1 sự kiện và khám phá nhé.
:::
...
Xảy ra khi có một người dùng mới cập nhật thông tin

```
{
  "events": [
    {
      "id": "evsklunazulefktuakcairrcl",
      "account_id": "acriviayfmabzskstrpq",
      "created": 1756088815623,
      "type": "user_info_updated",
      "data": {
        "user": {
          "id": "usskixczcisilewingagv",
          "account_id": "acriviayfmabzskstrpq",
          "attributes": [
            {
              "key": "created",
              "modified": 1755679499300,
              "by_type": "subiz",
              "datetime": "2025-08-20T08:44:59Z"
            },
            {
              "key": "seen",
              "modified": 1755680660643,
              "by_type": "subiz",
              "datetime": "2025-08-20T09:04:20Z",
              "by": "system"
            },
            {
              "key": "modified",
              "modified": 1756088815617,
              "by_type": "agent",
              "datetime": "2025-08-25T02:26:55Z",
              "by": "agrmzcjfkwhnlwjmgy",
              "user_value": "2025-08-20T09:04:20Z"
            },
            {
              "key": "first_interact",
              "modified": 1755679591822,
              "by_type": "subiz",
...
## Xử lý khi không nhận được dữ liệu từ Webhook {#xu-ly-khi-khong-nhan-duoc-du-lieu-tu-webhook}

Khi bạn nhận được cảnh báo về việc Webhook không nhận dữ liệu, nguyên nhân thường là phía bên thứ ba chưa tiếp nhận thành công dữ liệu mà Subiz gửi tới.

Hướng xử lý:

Hãy kiểm tra trạng thái hoạt động của webhook để đảm bảo rằng webhook đang hoạt động và luồng gửi- nhận dữ liệu vẫn thông suốt.

1. Đăng nhập vào Subiz.
2. Nhấn biểu tượng <b>⚙️ Cài đặt</b> ở góc trên cùng.
3. Chọn <b>Tích hợp &gt; Webhook</b>.
4. Kiểm tra trạng thái Webhook:

- <b>Hoạt động</b>: Kết nối đã hoạt động trở lại, bạn không cần làm gì thêm.
- <b>Chập chờn</b> hoặc <b>Gián đoạn</b>: Dữ liệu chưa được tiếp nhận thành công, cần báo bên nhận kiểm tra việc không nhận dữ liệu Webhook.

Nếu Webhook ở trạng thái <b>Chập chờn </b>hoặc <b>Gián đoạn</b>, bạn xử lý như sau:

1. Nhấn <b>Chỉnh sửa Webhook</b>.
2. Kéo xuống mục <b>Những lần gọi gần đây</b>.
3. Mở lần gọi mới nhất và chọn tab <b>Phản hồi</b>.
4. Chụp nội dung phản hồi cho đội kỹ thuật của bên thứ ba để họ có thêm thông tin kiểm tra xử lý việc không nhận dữ liệu.

Khi kết nối thông suốt trở lại, Webhook trên Subiz sẽ hiển thị trạng thái <b>Hoạt động</b> và các sự kiện trước đó chưa gửi thành công sẽ được Subiz gửi lại.
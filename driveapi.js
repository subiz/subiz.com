...
//		out.news = header.entries
	...
	lo.map(hot, (entry) => {
		...
		let sidebarcls = ''
		//		if (entry.path_lower == '/index') return // hide this specific document
		data =
			`---
...
` + data
	...
	let copydata = fs.readFileSync('./docs/10-lam-quen-voi-subiz/2-cac-goi-dich-vu.mdx')
...
`
	fs.writeFileSync('./docs/index.mdx', copydatas.join('---'), {encoding: 'utf8'})
	// hide /1000 tong hop
